...
const VideoHeader = (props) => {
    ...
    const getData = async () => {
        try {
            const value = await AsyncStorage.getItem('videoPlayingTitle');
            if (value !== null) {
    ...
    return (
        <SafeAreaView style={{ marginTop: StatusBar.currentHeight || 0 }}>
            <View style={styles.header}>
                ...
                <Text style={[styles.txtInput,styles.centerVertical]}>HAHAHA</Text>
                <View style={{ flex: 1 }} />
                <Ionicons style={{ marginHorizontal: 7 }}  name="download-outline" color="black" size={30} onPress={() => null} />
            </View>
        </SafeAreaView>
    )
}
